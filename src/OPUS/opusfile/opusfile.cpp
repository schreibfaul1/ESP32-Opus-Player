            ...
            if(last == OP_FALSE)
                ...
                if(!op_lookup_serialno(serialno, serialnos, nserialnos)) {
                    ...
                    if(nsr < _csr) {
                        ...
                        _sr[nsr].offset = last;
                        assert(_of->offset-last>=0); assert(_of->offset-last<=OP_PAGE_SIZE_MAX);
...
static void op_update_gain(OggOpusFile *_of) {
    ...
    gain_q8 = max(-32768, min(gain_q8, 32767));
    assert(_of->od!=NULL);
...
int opus_tags_get_gain(const OpusTags_t *_tags, int *_gain_q8, const char *_tag_name, size_t _tag_len) {
    ...
    for(ci = 0; ci < ncomments; ci++) {
        assert(_tag_len<=(size_t)INT_MAX);
